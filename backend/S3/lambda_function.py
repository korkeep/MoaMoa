...
def lambda_handler(event, context):
    ...
    path = "https://khu-static-s3.s3.ap-northeast-2.amazonaws.com/" # S3 Default Path
    table = event['table']
    ...
    # Original Image Link: path+name
    # Thumbnail Image Link: path+name+"_t"
    cursor = conn.cursor()
    sql = "INSERT INTO " + table + " (`index`, `explain`, `singer`, `hashtag`, `view`, `date`, `o_link`, `t_link`) VALUES (null, %s, %s, %s, '0', %s, %s, %s);"
    cursor.execute(sql, (explain, singer, hashtag, date, path+name, path+name+"_t"))
